...
class SearchableMixin(object):
    ...
    def search(cls, expression, page, per_page):
        print(expression, page, per_page)

        ids, total = query_index(cls.__tablename__, expression, page, per_page)
        if total == 0:
            return [], 0

        # https://stackoverflow.com/a/6332081/904393
        when = []
        for i in range(len(ids)):
            when.append((ids[i], i))

        # cls.id - колонка "id" в таблице класса в базе данных
        # строка cls.id.in_(ids) эквивалентна SQL-запросу "WHERE id IN (1, 2, 3)"
        # метод case обеспечивает соответствие порядка результатов запроса порядку переданных id
